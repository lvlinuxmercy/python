...
f = open(r'/pythondata//aliyw.sql', encoding='gbk')  # 查询sql所在的目录
daily_print = []
for line in f:
...
def dsffile(time,appkey):     # 对第三方定义一个额外的函数，因为要区分客户
    f = open(r'/pythondata//dsfyw.sql', encoding='gbk')
    dsf_daily_print = []
...
dm_dsf = dsffile(time=time, appkey='SF')   # 东盟第三方业务查询
tr_dsf = dsffile(time=time, appkey='TR')   # 天润业务
wl_dsf = dsffile(time=time, appkey='HZ')   # 微聊业务
